...
void setup(void)
{
  ...
  lcd.print("RIOT1 v1.0",LEFT,0);     // Display Splash Page
...
void loop()
{
  ...
  dtostrf(Moisture1_p, 3, 0, M1_b);
  dtostrf(Moisture2_p, 3, 0, M2_b);
  dtostrf(Moisture3_p, 3, 0, M3_b);
  dtostrf(Update_Sequence, 6, 0, SE_b);
         
  Serial.print(F("RIOT1:  1.0"));             // Node Type and Software Version
...
{
  int percentValue = 0;
  percentValue = map(value, 164, 65, 0, 100);
